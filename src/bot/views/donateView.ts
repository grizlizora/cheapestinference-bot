...
export function renderDonateText(ctx: BotContext, userTotalStars = 0): string {
  ...
  if (userTotalStars > 0) {
    statusBadge =
      ctx.lang === "uk"
        ? `\n\n${diamondIcon} <b>Ваш статус:</b> ${starIcon} <b>${userTotalStars} Stars задоначено</b> (Пріоритетна доставка сповіщень увімкнена!)`
        : ctx.lang === "ru"
        ? `\n\n${diamondIcon} <b>Ваш статус:</b> ${starIcon} <b>${userTotalStars} Stars задоначено</b> (Приоритетная доставка уведомлений включена!)`
        : `\n\n${diamondIcon} <b>Your Status:</b> ${starIcon} <b>${userTotalStars} Stars donated</b> (Priority alert delivery active!)`;
  }
  ...
  if (ctx.lang === "uk") {
    return clampMessageText(
      ...
      `Якщо бот допомагає вам вчасно зловити потрібний слот або ви просто хочете висловити подяку автору за розробку — ви можете надіслати символічний донат у <b>Telegram Stars (${starIcon})</b> чи пригостити кавою! ${coffeeIcon}\n\n` +
      `${starIcon} <b>Пріоритет у сповіщеннях:</b>\n` +
      ...
      `Оберіть суму подяки в <b>Telegram Stars (${starIcon})</b>:`
    );
  } else if (ctx.lang === "ru") {
    return clampMessageText(
      ...
      `Если бот помогает вам вовремя поймать нужный слот или вы просто хотите сказать спасибо автору за разработку — вы можете отправить символический донат в <b>Telegram Stars (${starIcon})</b> или угостить кофе! ${coffeeIcon}\n\n` +
      `${starIcon} <b>Приоритет в уведомлениях:</b>\n` +
      ...
      `Выберите сумму благодарности в <b>Telegram Stars (${starIcon})</b>:`
    );
  } else {
    return clampMessageText(
      ...
      `If this bot has helped you claim rare slots or you'd like to show appreciation to the developer, you can leave a friendly tip via <b>Telegram Stars (${starIcon})</b> or buy a coffee! ${coffeeIcon}\n\n` +
      ...
      `Select your tip in <b>Telegram Stars (${starIcon})</b>:`
    );
  }
}
