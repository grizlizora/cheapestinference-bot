...
export function renderDonateText(ctx: BotContext, userTotalStars = 0): string {
  ...
  const rocketIcon = icon("event_slot_drop");
  ...
  if (meta.isAdmin) {
    statusCard =
      ctx.lang === "uk"
        ? `\n\n👑 <b>Ваш статус:</b> <b>Адміністратор / Автор</b>\n• <b>Пріоритет:</b> ⚡ Черга P0 (Миттєва доставка)\n• <b>Імунітет:</b> ♾️ Безстроково активний`
        : ctx.lang === "ru"
        ? `\n\n👑 <b>Ваш статус:</b> <b>Администратор / Автор</b>\n• <b>Приоритет:</b> ⚡ Очередь P0 (Мгновенная доставка)\n• <b>Иммунитет:</b> ♾️ Бессрочно активен`
        : `\n\n👑 <b>Your Status:</b> <b>Admin / Creator</b>\n• <b>Priority:</b> ⚡ P0 Instant Dispatch\n• <b>Immunity:</b> ♾️ Lifetime Active`;
  } else if (userTotalStars > 0) {
    statusCard =
      ctx.lang === "uk"
        ? `\n\n${meta.iconHtml} <b>Ваш статус:</b> ${meta.rankTitle} (<b>${userTotalStars} Stars</b>)\n• <b>Пріоритет:</b> ${meta.priorityTitle}\n• <b>Утримання:</b> ${meta.retentionText}`
        : ctx.lang === "ru"
        ? `\n\n${meta.iconHtml} <b>Ваш статус:</b> ${meta.rankTitle} (<b>${userTotalStars} Stars</b>)\n• <b>Приоритет:</b> ${meta.priorityTitle}\n• <b>Удержание:</b> ${meta.retentionText}`
        : `\n\n${meta.iconHtml} <b>Your Status:</b> ${meta.rankTitle} (<b>${userTotalStars} Stars</b>)\n• <b>Priority:</b> ${meta.priorityTitle}\n• <b>Retention:</b> ${meta.retentionText}`;
  }
  ...
  if (ctx.lang === "uk") {
    return clampMessageText(
      ...
      `• 🛡️ <b>Розширене утримання:</b> Кожна зірка додає дні активного моніторингу без необхідності щотижневих заходів у бот.${statusCard}\n\n` +
    ...
    return clampMessageText(
      ...
      `• 🛡️ <b>Расширенное удержание:</b> Каждая звезда продлевает активный мониторинг без обязательных визитов.${statusCard}\n\n` +
    ...
    return clampMessageText(
      ...
      `• 🛡️ <b>Extended Retention:</b> Every Star adds active monitoring days without 14-day inactivity limits.${statusCard}\n\n` +
